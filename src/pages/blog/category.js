...
const Category = (props) => {
  ...
  const categoriesLoading = useSelector(
    (state) => state.blog.categoriesLoading
  );
  ...
  const handleInputChange = (obj) => {
    ...
    const value =
      typeof obj === "string"
        ? obj.replace(/[\r\n\v]+/g, "")
        : obj.target.value.replace(/[\r\n\v]+/g, "");
    document.getElementsByClassName("ris-input")[0].value = value;
    setCategoryInput(value);
    asyncChangeCallback(
      value,
      obj.nativeEvent.inputType === "insertLineBreak",
      blogForm?.values?.categories
    );
  };
  return (
            ...
            selectedItems.map((item, index) => {
              return (
                <li
                  ...
                  onClick={(event) => makeItDefault(event, item)}
                  key={index}
                >
                  {item.name}
                  <a
